...
void	Help::execute(ft::User *sender, const std::string& msg)
{
	...
	if (this->_cmd.empty()) {
		sender->send(NOTICE(sender->nick(), "HELP [<command>]"));
		sender->send(NOTICE(sender->nick(), "List of all available commands:"));
		for (std::map<std::string, Command *>::iterator cmd = commands.begin(); cmd != commands.end(); ++cmd) {
			if (cmd->first == "DEBUG") {
				continue;
			}
			sender->send(NOTICE(sender->nick(), cmd->second->name()));
		}
	} else {
		...
		try {
			...
			sender->send(NOTICE(sender->nick(), cmd->name() + ": " + cmd->description()));
		}
